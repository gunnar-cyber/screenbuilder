...
class CrowdinAPI {
    ...
    async readSourceFile() {
        ...
        try {
            ...
            const enJson = Function(`return ${objectLiteral}`)();
    ...
    async checkBuildStatus(projectId, buildId) {
        const statusResponse = await fetch(`${this.API_URL}/projects/${projectId}/translations/builds/${buildId}`, {
    ...
    async waitForBuild(projectId, buildId) {
    ...
    async createBuild(projectId, branchId) {
        console.log('\n🔄 Creating build...');
        const buildResponse = await fetch(`${this.API_URL}/projects/${projectId}/translations/builds`, {
            method: 'POST',
            headers: {
        ...
        const { data: build } = await buildResponse.json();
        console.log('🔄 Build initiated successfully:', build);
        console.log(`📦 Build ID: ${build.id}`);
        return build;
    }
    async uploadSourceStrings(projectId, content) {
        let fileId;
        let branchId;
        try {
            ...
            const storageResponse = await fetch(`${this.API_URL}/storages`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${this.API_TOKEN}`,
            ...
            const { data: storage } = await storageResponse.json();
            const storageId = storage.id;
            console.log('✓ Storage created successfully');
            // Step 3: Check for existing branch
            console.log('🌿 Checking for existing branch...');
            const branchesResponse = await fetch(`${this.API_URL}/projects/${projectId}/branches`, {
                headers: { Authorization: `Bearer ${this.API_TOKEN}` }
            });
            const { data: branches } = await branchesResponse.json();
            ...
            if (existingBranch) {
                ...
                const createBranchResponse = await fetch(`${this.API_URL}/projects/${projectId}/branches`, {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${this.API_TOKEN}`,
                        'Content-Type': 'application/json'
                ...
                const { data: newBranch } = await createBranchResponse.json();
                branchId = newBranch.data.id;
                console.log(`✓ Created new branch (ID: ${branchId})`);
            }
            // Step 4: Check for existing files
            console.log('🔍 Checking for existing files...');
            const filesResponse = await fetch(`${this.API_URL}/projects/${projectId}/files?branchId=${branchId}`, {
                headers: { Authorization: `Bearer ${this.API_TOKEN}` }
            });
            const { data: files } = await filesResponse.json();
            console.log('Files in branch:', files.map((f) => ({
                name: f.data.name,
                id: f.data.id,
                branchId: f.data.branchId
            })));
            const existingFile = files.find((file) => file.data.name === this.FILENAME);
            if (existingFile) {
                // Update existing file
                console.log(`📝 Updating existing file (ID: ${existingFile.data.id})...`);
                const updateResponse = await fetch(`${this.API_URL}/projects/${projectId}/files/${existingFile.data.id}`, {
                    method: 'PUT',
                    headers: {
                        Authorization: `Bearer ${this.API_TOKEN}`,
                        'Content-Type': 'application/json'
                ...
                const createFileResponse = await fetch(`${this.API_URL}/projects/${projectId}/files`, {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${this.API_TOKEN}`,
                        'Content-Type': 'application/json'
                ...
                const { data: newFile } = await createFileResponse.json();
                fileId = newFile.id;
    ...
    async updateTypeScriptFile(jsonPath, tsPath) {
    ...
    async downloadAndExtractBuild(projectId, buildId) {
        console.log('\nDownloading build...');
        const downloadResponse = await fetch(`${this.API_URL}/projects/${projectId}/translations/builds/${buildId}/download`, {
            headers: { Authorization: `Bearer ${this.API_TOKEN}` }
        });
        ...
        try {
            ...
            const listFiles = async (dir, indent = '') => {
            ...
            const findTranslationFile = async (dir, lang) => {
                const entries = await fs.readdir(dir, { withFileTypes: true });
                for (const entry of entries) {
                    const fullPath = path.join(dir, entry.name);
                    if (entry.isDirectory()) {
    ...
    async getTranslationDetailsBasedOnSource(projectId, fileId) {
        console.log('\n🔍 Getting translation status for all languages...');
        const progressResponse = await fetch(`${this.API_URL}/projects/${projectId}/files/${fileId}/languages/progress`, {
            headers: { Authorization: `Bearer ${this.API_TOKEN}` }
        });
        ...
        const stats = [];
        for (const lang of this.supportedLanguages) {
            const langProgress = progressData.find((item) => {
    ...
    async main() {
        try {
            ...
            let content = await this.jsonHandler.readJson(this.paths[`${CROWDIN.DEFAULT_SOURCE_LANG}JsonPath`]);
            if (this.dataFromTypeScript) {
